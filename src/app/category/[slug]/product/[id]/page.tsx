...
export default function ProductDetailsPage({ params }: ProductPageProps) {
  ...
  const product = products.find((p) => p.id === id && p.category === slug);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10 flex flex-col gap-8">
        ...
        <div className="overflow-hidden rounded-2xl shadow-md" ref={emblaRef}>
          <div className="flex">
            {images.map((img: string, idx: number) => (
              <div key={idx} className="flex-[0_0_100%] relative aspect-[4/3]">
                <img
                  src={img}
                  alt={product.title}
          ...
          {images.map((_: string, idx: number) => (
            <button
              ...
              className={classNames(
                ...
                idx === selectedIndex ? 'bg-blue-600 scale-110' : 'bg-gray-300 hover:bg-gray-400',
              )}
            />
          ))}
        </div>
      ...
      <div className="flex flex-col sm:flex-row justify-center md:justify-start gap-4">
        ...
        >
          <Heart className={classNames('h-5 w-5', isFavourite ? 'fill-current text-white' : '')} />
